...
function Doctors() {
    ...
    return (
        <div className='container'>
           ...
           <h1 className='text-center mb-5 fw-bold'>Our Dedicated Doctors Team</h1>
            <div className='row row-cols-lg-3 g-4'>
            {
                doctors.map(doctor =>(
                    ...
                    <h3 className="card-title my-3 text-dark">{doctor.name}</h3>
                    <h5 className="card-title my-3 text-dark">{doctor.dept}</h5>
                    <p className='text-muted'>Experience: {doctor.experience}+ years</p>
                    <h5 className="card-title my-4">Visiting fee: ${doctor.price}</h5>
                    <Link to="/contact"><button className='btn btn-dark mb-3'>Get Appoinment</button></Link>
                    </div>
                    </div>
                    </article>
                ))
